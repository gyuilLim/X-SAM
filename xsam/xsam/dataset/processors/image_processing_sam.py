...
from transformers.image_transforms import center_crop, convert_to_rgb, pad, resize, to_channel_dimension_format
...
if is_tf_available():
    ...
    from tensorflow.experimental import numpy as tnp  # type: ignore
    from transformers.tf_utils import flatten, shape_list  # type: ignore
...
class SamImageProcessor(BaseImageProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mask_size = mask_size if mask_size is not None else {"longest_edge": 1024}
        mask_size = (
        ...
        mask_pad_size = mask_pad_size if mask_pad_size is not None else {"height": 1024, "width": 1024}
        ...
        self.crop_size = crop_size if crop_size is not None else {"height": 1024, "width": 1024}
        self.do_flip = do_flip
        self.flip_ratio = flip_ratio
        self.flip_direction = flip_direction if flip_direction is not None else "horizontal"
        self.do_pad = do_pad
    ...
    def crop_image(
        ...
        """
        Crop an image to `(size["height"], size["width"])`.
        """
        crop_height, crop_width = crop_size["height"], crop_size["width"]
        input_height, input_width = get_image_size(image, channel_dim=input_data_format)
        crop_height = min(crop_height, input_height)
        crop_width = min(crop_width, input_width)
        crop_size = (crop_height, crop_width)
        return center_crop(image, crop_size, data_format, input_data_format)
    ...
    def flip_image(
        ...
        assert direction in ["horizontal", "vertical", "diagonal", None], f"Invalid flip direction: {direction}"
        ...
        if direction == "horizontal":
            ...
            new_image = np.flip(image, axis=1)
        elif direction == "diagonal":
            new_image = np.flip(image, axis=(1, 2))
        else:
            new_image = image
    ...
    def _get_longest_edge_preprocess_shape(self, old_shape: Tuple[int, int], longest_edge: int):
        """
    ...
    def _get_shortest_edge_preprocess_shape(self, old_shape: Tuple[int, int], shortest_edge: int):
        """
        Compute the output size given input size and target short side length.
        """
        oldh, oldw = old_shape
        scale = shortest_edge * 1.0 / min(oldh, oldw)
        newh, neww = oldh * scale, oldw * scale
        newh = int(newh + 0.5)
        neww = int(neww + 0.5)
        return (newh, neww)
    ...
    def _get_random_scale_preprocess_shape(
        self, old_shape: Tuple[int, int], min_scale: float, max_scale: float, target_size: Tuple[int, int]
    ):
        """
        Compute the output size given input size and random scale.
        """
        if isinstance(target_size, int):
            target_size = (target_size, target_size)
        oldh, oldw = old_shape
        scale = random.uniform(min_scale, max_scale)
        target_scale_size = (int(target_size[0] * scale + 0.5), int(target_size[1] * scale + 0.5))
        target_scale = min(target_scale_size[0] / oldh, target_scale_size[1] / oldw)
        newh, neww = oldh * target_scale, oldw * target_scale
        newh = int(newh + 0.5)
        neww = int(neww + 0.5)
        return (newh, neww)
    ...
    def _get_output_size(
        ...
        if "longest_edge" in size:
            ...
            output_height, output_width = self._get_shortest_edge_preprocess_shape(input_size, size["shortest_edge"])
    ...
    def _get_flip_direction(self, flip_ratio: float, flip_direction: str):
        if flip_direction == "horizontal" and random.random() < flip_ratio:
            return "horizontal"
        elif flip_direction == "vertical" and random.random() < flip_ratio:
            return "vertical"
        elif flip_direction == "diagonal" and random.random() < flip_ratio:
            return "diagonal"
        else:
            return None
    ...
    def resize(
        ...
        **kwargs,
        ...
        if "longest_edge" in size:
            ...
            raise ValueError(
                f"The `size` dictionary must contain the key `longest_edge` or `height` and `width`. Got {size.keys()}"
            )
    ...
    def _preprocess(
        ...
        if do_flip:
            image = self.flip_image(image=image, direction=flip_direction, input_data_format=input_data_format)

        if do_crop:
            image = self.crop_image(image=image, crop_size=crop_size, input_data_format=input_data_format)
        reshaped_input_size = get_image_size(image, channel_dim=input_data_format)

        if do_pad:
            image = self.pad_image(
                image=image, pad_size=pad_size, input_data_format=input_data_format, constant_values=constant_values
            )
    ...
    def preprocess(
        ...
        Args:
            ...
            crop_size (`Dict[str, int]`, *optional*, defaults to `self.crop_size`):
                Controls the size of the image after `crop`. The image is cropped to `crop_size["height"]` and
                `crop_size["width"]` if `do_crop` is set to `True`.
            do_flip (`bool`, *optional*, defaults to `self.do_flip`):
                Whether to flip the image.
            flip_ratio (`float`, *optional*, defaults to `self.flip_ratio`):
                Probability of flipping the image.
            flip_direction (`str`, *optional*, defaults to `self.flip_direction`):
                Direction to flip the image. Can be one of: "horizontal", "vertical", "diagonal".
        ...
        size = get_size_dict(max_size=size, default_to_square=False) if not isinstance(size, dict) else size
        output_size = self._get_output_size(images, size, input_data_format)
        size = output_size
        ...
        flip_direction = flip_direction if flip_direction is not None else self.flip_direction
        flip_direction = self._get_flip_direction(flip_ratio, flip_direction)
        ...
        crop_size = get_size_dict(crop_size, default_to_square=False) if crop_size is not None else None
